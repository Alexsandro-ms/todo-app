...
const createTask = async (req, res) => {
    try {
        ...
        const dueDataInput = req.body.dueData; 
        ...
        if(dueDataInput !== undefined){
            const clientDueDate = new Date(dueDataInput);
            clientDueDate.setHours(clientDueDate.getHours());
            if (isNaN(dueDate)) {
                return res
                    .status(400)
                    .json({
                        error: "The data provided is invalid. Provide valid data.",
                    });
            }
            taskData.dueData = dueDate;
        }
...
const listTasks = async (req, res) => {
    try {
        ...
        const userTasks = await TaskRepository.listByUser(
            user_id,
            page,
            perPage
        );
...
const listTaskToday = async (req, res) => {
    try {
        ...
        const completedTasks = await TaskRepository.findTodayTasks(
            user_id,
            startOfDay,
            endOfDay
        );
